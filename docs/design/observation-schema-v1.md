...
5. **Libraries are not serialized** — *amended by 5a below.* Own-library-as-remaining-counts (§2) is derivable at transform time: decklist (via deck name + pool manifest) minus owned, token-flagged entities observed in logged zones. Saves ~2×90 entities per observation — the difference between ~4KB and ~10KB records. Known-position library cards (scry-to-top) are a v1 gap, noted for v2 entity promotion; the scry decisions themselves are still logged. Sanity check for the derivation (tokens, copies) rides D3's label-sanity pass.
   - **5a (M1 D3 amendment): library *tops* with engine look permission are serialized.** D3's 320-game validation found 63 labels hosted on library-top cards (Mystic Forge / Oracle of Mul Daya / Courser of Kruphix — `getAvailableCards` always includes every library top; `canPlay()` gates castability). Rule: each player's top library card is written as an entity with `"z":"library"` whenever `mayPlayerLook(controller)` (the engine's own truth — card scripts grant `MayLookAt$ You` or `MayLookAt$ Player`); `vis` is **always stated** on library rows (`"all"` when every player may look, else `"c"`); a library row without `vis` is hidden to all (transform default). Additionally, priority decs union any option-host card from an unwalked zone into the observation (`vis` may then be `"none"`), so a label's host is in its own observation *by construction*. Deeper library knowledge (scry-to-top order) remains the v2 gap. Library derivation note: transform-side remaining-count derivation must not double-count serialized library-top entities.
...
**Visibility defaults by zone:** battlefield/graveyard/exile/stack/command → public; hand → controller-only; library → none (only tops with look permission are logged, decision 5a, and those always carry `vis`). `vis` is logged only on deviation: `"all"` (revealed hand card, publicly-revealed library top), `"c"` (face-down battlefield/exile or Mystic Forge-class library top — controller knows), `"none"` (face-down exile nobody may inspect; option-host extras). The transform's information-set rule: perspective player P sees `n` iff the effective visibility includes P; invisible entities collapse into (zone, controller) count aggregates. Hidden-zone ground truth thus stays in the record for M2 belief labels without ever reaching the M1 policy input.
...
```jsonc
// dec for chooseSpellAbilityToPlay now carries structured opts: TIMING-LEGAL
// CANDIDATES (same scan as the bridged path — canPlay() spells + land drops;
// payability deliberately NOT filtered, see the D3 amendment below),
// materialized because replay drift forbids recomputing legality later and
// the gate metric's single-legal-option exclusion needs it. Pass is NOT an
// entry; a null ret is the pass.
...
Facts the shape encodes (census-proven, archaeology-verified): the returned SA already **is** the optional-cost copy (`GameActionUtil.addOptionalCosts`), so `opt` reads off it; X and targets are pre-set during `canPlaySa` evaluation; `modes` is usually **absent for cast spells** — they bind later at `chooseModeForAbility`, which is its own labeled `dec`/`ret` (mode subs serialize CastPlan-shaped too, carrying their targets). Option entries match the chosen plan at host-entity level. The option scan iterates `getOriginalAndAltCostAbilities(getSpellAbilities(...))` — the AI's own iteration set — so alternative/additional-cost variants each appear (a spell playable *only* via its alternative cost, Snuff Out's 4 life, must be an option — D2's first validator catch).

**D3 amendment — options are timing-legal candidates, payability unfiltered.** D2's scan additionally filtered on `canPayCost` over the raw SA; D3's 320-game validation proved that filter both *wrong* (11 chosen-not-in-options errors: cost reductions and additional costs are priced only after `canPlaySa` sets targets/X — "can only be checked late", the engine's own comment — so Mystical Dispute/Dargo/X-spell picks were masked out) and *expensive* (it duplicates mana enumeration against the AI's own evaluation every window). The mask is now `canPlay()`-only — the same predicate the AI itself requires (`Spell.canPlay() == canPlayFromHost() != null`), making the option set a **superset of the expert's castable actions by construction**. Affordability is the model's to learn (it prices costs anyway to emit CastPlans); the gate metric's single-legal-option exclusion is defined on this candidate basis (marginally stricter — fewer windows excluded). `-Danvil.scan.paycheck=on` restores the old filter for comparison runs only.
