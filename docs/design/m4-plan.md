...
1. **Curation regeneration, new engine:** rerun `early_doom.py` + curation on D1's closing read. Expect the same *shape* (the addressable fraction and crash-window structure), not the same numbers — everything re-prices on the new scale. The doom ceiling gets its first new-scale read here.
...
1. **The re-baseline question is resolved:** the adaptation probe has run under guards, its best checkpoint has a paired 2,000-game corrected read, and an ADR records the mechanism verdict and the M4 baseline number.
