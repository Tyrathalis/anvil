...
### D3 — Critic calibration (promoted secondary, cycle-3-facing) — RESOLVED 2026-08-05 ([ADR-0036](../decisions/ADR-0036-d3-critic-calibration.md)): **split verdict.** Absolute scale FIXED — isotonic remap takes held-out ECE 0.30–0.36 → 0.025–0.048 (≈ the repeat-noise floor); ADOPTED era-scoped for cycle-3 curation/doom labels. Ranking NOT fixable: held-out Spearman 0.26–0.29 vs an achievable 0.94–0.97 (repeat-measure ceiling); post-remap residual by bin = winnable −0.56 / coin −0.30 / lost +0.16, global across decks/turns ⇒ **the rider's reading (2): representation-level blindness — the measured M6 candidate.** Ante riders not triggered (curation-side adoption only). Audit invariant mechanized (`scripts/critic_calibration.py` appends any future map/sweep run).

The banked ground truth (~1,900 labels from the cycle-1 map + sweep, plus D1's new labels — ~3,700 total after cycle 2) vs the critic that read 0.58 where rollouts said 0.24. Bounded: a calibration/fine-tune pass on the label set, measured on held-out labels (calibration error + rank correlation), against the current eval/Ante critic as baseline. Adopt for cycle-3 curation/eval on demonstrated improvement; documented negative otherwise. Explicitly firewalled from cycle 2 (no recipe delta). Riders attached if it lands: the M4-carried Ante correctness items (draw-poison coverage 69%, re-deal re-anchoring, node-level draw bias). Also the natural home for mechanizing the "value function continuously audited against rollouts" invariant (every future map/sweep run appends to the calibration set).

**Residual-decomposition rider (user-agreed 2026-08-05, post-ADR-0035):** D3 doubles as the diagnostic for the deeper question the M3–M5 falsification record raises (levers that rearrange the same signal fail; levers that change what the gradient sees pay). After fitting the best monotone recalibration (Platt + isotonic, held-out by game), decompose the residual by state class (turn bucket, ground-truth bin, deck/commander, cycle/era). Two pre-registered readings: (1) monotone remap absorbs most of the error ⇒ bookkeeping-level miscalibration — clean win for curation/Ante, and candidate for recalibrated value targets in-loop; (2) large state-dependent residual survives any monotone remap ⇒ the frozen-encoder representation cannot see what makes those position classes lost (the commander-tax/monarchy gap class) ⇒ representation work becomes the measured M6 candidate. Label-noise floor estimated from repeat measurements of the same positions (map labels vs evalset baseline re-measures, same policy argmax) so "residual" is judged against achievable, not zero. Era discipline: rollout truth is policy-conditional — cycle-1 labels (iter-009 completions) and cycle-2 labels (iter-019 completions) are fit/evaluated per-era and pooled only if the per-era calibrations agree.
