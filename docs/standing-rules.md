...
- **The search values an option under the heuristic's realization; a policy that learns WHICH option cannot execute it by that alone** — search copies realize a directed option through the AI's planner, the mainline through the model's CastPlan; a distilled policy over-generalized the search's overrides into casts the AI itself refuses (−7 to −9 pp at full dose); the with-lookahead advantage partly IS that planning, a Build 5 risk: the decoder must learn the realized plans (route (b)) ([ADR-0111](decisions/ADR-0111-m12-build4-opening-targets-surface-site.md)).
