...
- **First measurements (2026-07-03 soak, 6K games/8.2h single JVM):** forward-play is leak-free (heap-after-GC flat; LearnForge's killer confirmed dead); **734 games/h single-thread** heuristic-vs-heuristic; ~2.3 GB allocations/game (the scaling risk); ~26 MB/h native RSS drift (recycle workers routinely); 0.4% of games hit the 120s draw clock. Multi-worker scaling curve and fork-fidelity test still pending — those complete M0's measurement set.
