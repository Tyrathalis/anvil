...
- **M0 design phase complete.** Remaining M0 implementation scope: `.proto` + Python random-legal server + gRPC `AnvilBridge` impl, orchestrator in `anvil/bridge/`, then the bridge-tax measurement (arms: local-random control vs gRPC — same RNG answers, isolates transport).
