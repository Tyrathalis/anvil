...
- **Research — M12 DRAFT, scoping session next ([m12-plan.md](docs/design/m12-plan.md)): SEARCH AS THE BEHAVIOR POLICY.** The generating policy = the network + a bounded lookahead through the engine, leaves valued by the network's own MASKED head (full-vis never acts); picks amortized by distillation + PG at store scale. One directive, three knobs — depth d, breadth b, leaf rolls K (`-lookahead d b K`, Java; `anvil.value` on the server) — so deeper search is a flag. Build order: (0) directive + void-at-apply pre-filter + the DAY-ZERO paired read of with-lookahead vs the executor (**≥ +1.5pp GO / ≤ 0 → sharpen the value head, re-read once; a second ≤ 0 kills**) → (1) value-head sharpening → (2) the amortization run → (3) depth curve d=2 / K>1 at matched compute → (4) the 2,000-game read (network alone promotes; with-lookahead = the deployment ceiling). Forks A–F adjudicated at scoping. Routed by name: tutor targets, trigger order / combat damage, §3b stops, mull tuck; stack-entry tokens + semantic option content after Build 2 moves strength. Running record: the plan doc.
