...
- **M0 design phase complete.** Remaining M0 scope is implementation: `PlayerControllerAnvil` + `anvil` launcher in the fork (starting with the instrumented callback census), `.proto` + Python random-legal server + orchestrator in `anvil/bridge/`, then the three-arm bridge-tax measurement.
