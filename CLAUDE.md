...
- **Research — M12 RECHARTERED 09-06 ([ADR-0101](docs/decisions/ADR-0101-architecture-review-m12-recharter.md), [m12-plan.md](docs/design/m12-plan.md)); scoping session pins numbers next: SEARCH AS THE BEHAVIOR POLICY, built through to ONE BIG RUN.** The architecture review named the value function as the never-built central asset, the loop at ~10K games/run against a 1–3M design budget (throughput binding), the veto economy as an inexact legality mask, and the hybrid agent's excluded surfaces as an unread sum. Build order: (0) the engine bundle, one boundary — exact payability in the mask, game-time/repetition caps, the budgeted anytime search directive (evaluations, pivotality-allocated with a uniform floor), the surface enumerators → (1) the value head on rollout composites + full-vis targets, permanent rollout anchor → (2) search on the masked head + **the ONE gate: the day-zero paired read (≥ +1.5pp GO / ≤ 0 kill-candidate) with a heuristic+lookahead control arm** → (3) every §3d′ surface (three answer shapes: entity set / ordering / name; natural line always in the option set; smoke reads, no gates) → (4) stack-entry tokens + embedded ability text → (5) **the big run ≥ 300K games with a power statement**, read mid-run for the network-alone vs with-lookahead gap (kill: with-lookahead climbs, network-alone flat) → (6) the 2,000-game read. Search = drill-finding (Grindstone folds in); the ladder of own checkpoints + ε-margin difficulty dial now, the skill token after. Forks B–F adjudicated; A, G (Pauper testbed), H (ship `iter-019` on Android) at scoping. Running record: the plan doc.
- **Playable fork ([worklist](docs/design/playable-fork-worklist.md)): release v24 `fe86deded9` (`2.0.15-SNAPSHOT-09.05`).** Founding worklist items 1–8 built, published, user-verified; v23 = the upstream sync onto `89806371a4d`; v24 fixed the Android res refresh (first real mobile-delta run pending on device). Branch `playable`, worktree `../forge-play`; zero delta on the research fork. Release: `scripts/release-playable.sh` → `daily-snapshots` prerelease, **publish MUST pin `-R Tyrathalis/forge`**. Residue: T2 live-rescale. Multiplayer hardening CLOSED 07-30 ([record](docs/design/multiplayer-hardening.md)).
