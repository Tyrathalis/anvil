#!/usr/bin/env bash
# d6-run16 — the M7 C-bundle run, take 3 (ADR-0057: run15's share guard
# adjudicated the frozen-w_seq design — PG mass declines while the hinged
# L_seq does not, so seq_share drifted 0.21 -> 0.27+ toward the 0.3 guard
# with NO seq-term misbehavior and NO behavioral damage, hold-then-cast
# flat). Base = launch_d6_run15.sh verbatim; delta = per-iteration w_seq
# recalibration (now the driver default; the hinge makes it safe) + fresh
# seed_base 20260817. Restarts from iteration 0.
# Gate: standing combined paired read vs 0.5373 +/- 0.0112 (ADR-0055, era
# d798917ae5). Pre-registered secondary read (ADR-0056): holding
# same-turn hold-then-cast toward heur ~0.345.
# ~2h45m/iter -> ~55h.
#
# Launch checklist (the detached-run-launches memory): setsid nohup this
# script; driver self-registers watchd; ALSO arm a persistent log monitor
# (GUARD HALT / ANOMALY / battery anomalies / Traceback / KL ABORT) +
# periodic telemetry glances (watch: per-iteration w_seq trajectory =
# the PG-mass proxy; seq_share holding ~0.1).
set -euo pipefail
cd "$(dirname "$0")/.."
exec uv run python -m anvil.training.selfplay \
  ...
  --workers 8 --chunk 30 --port 50063 --seed-base 20260817 \
