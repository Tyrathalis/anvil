...
P2_ARMS = "o1:crash:-1,o2:crash:-2,o3:crash:-3,o4:crash:-4,o5:crash:-5,o6:crash:-6,peak:peak:0"


WATCH_STATE = {"done": False, "phase_start": 0.0, "step": "startup", "stalled": False}
...
def _campaign_watchdog(stall_min: int) -> None:
    ...
    while not WATCH_STATE["done"]:
        ...
        pats = [
            ...
            str(ROOT / "data/runs" / "drill*" / "workers" / "*" / "labels.jsonl"),
        ...
        if quiet_min < stall_min:
            ...
            try:
                ...
                pass
        elif not WATCH_STATE["stalled"] or time.time() - last_alert > 4 * stall_min * 60:
...
def _phase_done(name: str) -> bool:
    if _marker(name).exists():
        print(f"[tranche-c2] phase {name} already done — skipping", flush=True)
        return True
    return False
...
def _label_dirs() -> list[str]:
    dirs = [f"{BASE}/p1-arms/arm-{t}" for t in ("o1", "o3", "o5", "o6", "peak")]
    dirs += [P2_MAP]
    dirs += [f"{BASE}/p2-arms/arm-{t}" for t in ("o1", "o2", "o3", "o4", "o5", "o6", "peak")]
    return [d for d in dirs if Path(ROOT / d / "drills.jsonl").exists()]


def _checkpoint(tag: str, pause_ok: bool) -> None:
    cmd = [sys.executable, "scripts/tranche_checkpoint.py", "--out", f"{BASE}/{tag}"]
...
def main() -> None:
    ...
    ap.add_argument(
        "--smoke", action="store_true", help="p1 limit 6 rows, 16 games/arm in p2, map limit 8"
    )
    a = ap.parse_args()
    ...
    threading.Thread(target=_campaign_watchdog, args=(ap_stall,), daemon=True).start()
    if not a.smoke:
        _watchd("register")
    ...
    try:
        ...
        if not _phase_done("p2-trace"):
            ...
            _run([sys.executable, "scripts/early_doom.py", "analyze", "--out", TRACE_ERA3])
            ...
            with dst.open("w") as f:
                for line in src.open():
                    ...
                    kept += 1
            print(
                f"[tranche-c2] p2 curation: {kept} train games "
        ...
        if not _phase_done("p2-drill"):
            _run(
                [
                    ...
                    "plan",
                    "--curation",
                    f"{BASE}/p2-curation-trainonly.jsonl",
                    "--out",
                    P2_MAP,
                    "--ckpt",
                    POLICY,
            ...
            _run([sys.executable, "-m", "anvil.grindstone", "report", "--manifest", P2_MAP])
            if not a.smoke:
                _run(
                    [
                        sys.executable,
        ...
        if not _phase_done("merge"):
            cmd = [
                ...
                MERGED,
                "--trace-era",
                "data/runs/early-doom-run11-i019-ext",
                "--trace-era",
                TRACE_ERA3,
                "--trace-d4",
                "data/runs/early-doom-run11-d4crit-ext",
                "--trace-d4",
                TRACE_D43,
            ]
            for d in _label_dirs():
        ...
        if not _phase_done("probe"):
            ...
            _run(
                ...
                + (
                    ...
                    if a.smoke
                    else ["--ns", "0,1,2,4", "--lrs", "3e-5", "--seeds", "0,1,2"]
                )
            )
            _finish("probe")

        wall_h = (time.time() - t0) / 3600
        report = json.loads((ROOT / FINAL_PROBE / "unfreeze-probe-report.json").read_text())
        best = max(report["cells"], key=lambda c: c["holdout_spearman"])
        notify(
            "tranche c2 CAMPAIGN DONE",
