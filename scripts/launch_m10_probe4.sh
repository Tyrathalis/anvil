#!/usr/bin/env bash
# m10-probe4 — the M10 training probe on the ADR-0088 GROUNDED DRIVER
# (m10-probe3 relaunch: probe3 GUARD-HALTED at iteration 0 on a FALSE
# memorize-guard trip — the guard compared per-trajectory row values
# (÷ traj_per_step) to the per-step calibration raw and could never
# pass; labs_early.json showed healthy per-step CE ~2.4 vs calib 2.68 and
# KL 0.010–0.016, the cleanest iteration 0 of the three probes. Guard
# re-based on per-step keys + iteration-min at 0.25x; fresh name/seed
# 20280831/port 50082 per ADR-0076; probe3 dir kept as forensic asset):
...
exec uv run python -m anvil.training.selfplay \
  ...
  --workers 8 --chunk 30 --port 50082 --seed-base 20280831 \
