...
# ONE worker: at 8 workers the bf16 micro-batch composition flips near-tie argmax picks (13/120 games
# diverged on IDENTICAL masks, 09-14) — an identity gate on a served arm serves batches of one.
PORT=${PORT:-50074}; CKPT=${CKPT:-$REPO/data/training/m12-build3-e3/last.pt}; GAMES=${1:-32}; WORKERS=${WORKERS:-1}
OUT=$REPO/data/runs/build3-memo-gate-w$WORKERS-g$GAMES; mkdir -p "$OUT"
...
python3 scripts/anvil_watchd.py register --name memo-gate-w$WORKERS --pid $$ --dir "$REPO/data/runs" --stall-min 45
...
for arm in off on; do
  ...
  ANVIL_EXTRA_JVM_OPTS="-Danvil.scan.sourcememo=$arm" nice -n 19 uv run python -m anvil.bridge.harness launch --pool --games "$GAMES" --games-per-pair 5 \
    --workers "$WORKERS" --bridge grpc:localhost:$PORT --census --obs --purpose "memo-gate-w$WORKERS-$arm" --jar "$JAR" \
...
A=$(ls -d $REPO/data/runs/memo-gate-w$WORKERS-off-* | tail -1); B=$(ls -d $REPO/data/runs/memo-gate-w$WORKERS-on-* | tail -1)
...
python3 scripts/anvil_watchd.py unregister --name memo-gate-w$WORKERS
