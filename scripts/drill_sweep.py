...
def _watchdog(out: Path, tags: list[str], stall_min: int,
              state: dict) -> None:
    ...
    while not state["done"]:
        ...
        for tag in tags:
            for lf in glob.glob(str(gs.RUNS_DIR / f"drill{tag}-*"
                                    / "workers" / "*" / "labels.jsonl")):
                newest = max(newest, os.path.getmtime(lf))
        stalled = time.time() - newest > stall_min * 60
        if stalled and not quiet:
            notify("anvil drill sweep STALLED",
                   f"no labels written in {stall_min} min ({out})")
            quiet = True
        elif not stalled:
            quiet = False
...
def _arm_rows(arm_dir: Path) -> dict[tuple, dict]:
    return {(r["store"], r["g"]): r
            for r in map(json.loads, (arm_dir / "drills.jsonl").open())}
...
def _summarize(out: Path, arms: list[dict]) -> dict:
    ...
    for a in arms:
        ...
        per_arm[a["tag"]] = {
            **a,
            ...
            "bins": rep["bins"],
            "mean_fired_t": round(sum(r["fired_t"] for r in rows.values())
    ...
    for a in arms[1:]:
        ...
        common = [(base[k], rows[k]) for k in base.keys() & rows.keys()
                  if base[k]["n"] > 0 and rows[k]["n"] > 0]
        deltas = [r["model_wins"] / r["n"] - b["model_wins"] / b["n"]
                  for b, r in common]
        trans = Counter(
            f"{gs._bin_of(b['model_wins'], b['n'])}->"
            f"{gs._bin_of(r['model_wins'], r['n'])}" for b, r in common)
        paired[a["tag"]] = {
            "n_pairs": len(common),
            "mean_wr_delta": round(sum(deltas) / len(deltas), 4)
                             if deltas else None,
            "transitions": dict(trans.most_common()),
        }

    summary = {
        "arms": {t: {k: v for k, v in d.items() if k != "_rows"}
...
def main() -> None:
    ...
    ap.add_argument("--bins", default="lost",
                    help="comma-joined ground-truth bins to re-drill")
    ap.add_argument("--arms", default="o0:crash:0,o2:crash:-2,o4:crash:-4,"
                    "peak:peak:0",
                    help="comma-joined tag:anchor:turn_offset specs; the "
                         "first arm is the paired baseline")
    ap.add_argument("--out", type=Path, required=True)
    ap.add_argument("--k", type=int, default=None,
                    help="completions per drill (default: the map's K)")
    ...
    wd = threading.Thread(target=_watchdog, daemon=True,
                          args=(a.out, [x["tag"] for x in arms],
                                a.stall_min, state))
    wd.start()
    ...
    try:
        for arm in arms:
            ...
            print(f"[sweep] arm {arm['tag']}: anchor={arm['anchor']} "
                  f"offset={arm['turn_offset']}")
            gs.plan(argparse.Namespace(
                curation=subset, out=str(arm_dir),
                ckpt=map_manifest["ckpt"], k=k, anchor=arm["anchor"],
                turn_offset=arm["turn_offset"], tag=arm["tag"],
                limit=a.limit))
