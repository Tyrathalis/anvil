...
st=$(python3 -c "import json; print(json.load(open('/home/tyrathalis/.local/state/anvil/runs/build4-rewarm.json'))['state'])" 2>/dev/null)
log "re-warm state: $st; launching the paired read"
NAME=b4rw CKPT=data/training/m12-build4-e1/last.pt CKPT_ALT="$RW" GAMES=300 WORKERS=24 SERVERS=2 PORT=50069 ARMS="on alt" \
  FORGE_ARGS="-modegate off" JAR="$REPO/data/runs/build4-targets/forge-targets-gate.jar" \
  uv run python -m anvil.runs launch --name build4-read-b4rw --dir "$REPO/data/runs/build3-surface-read-b4rw" --stall-min 60 -- bash scripts/build3_surface_read.sh
uv run python -m anvil.runs wait --name build4-read-b4rw; rc=$?
log "paired read rc=$rc: $(tail -1 data/runs/build3-surface-read-b4rw/chain.log | cut -c1-400)"
echo OK > "$OUT/DONE"
