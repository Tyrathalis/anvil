...
run_arm off "$TAGS_OFF" || { log "arm off FAILED"; finish 1; }
run_arm on || { log "arm on FAILED"; finish 1; }
uv run python scripts/build2_read.py arms --arm "off=$(cat $OUT/off.done)" --arm "on=$(cat $OUT/on.done)" --out "$OUT/read.json" >> "$OUT/read.log" 2>&1 || { log "read FAILED"; finish 1; }
