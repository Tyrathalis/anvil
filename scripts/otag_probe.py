...
UA = {"User-Agent": "anvil-research/0.1 (non-commercial; otag probe)",
...
def _search_tag(tag: str) -> list[str] | None:
    ...
    names: list[str] = []
    url = (f"{API}?q={urllib.parse.quote(f'otag:{tag} legal:duel')}"
           "&unique=cards")
    while url:
...
def fetch(args: argparse.Namespace) -> None:
    ...
    for group, tags in GROUPS.items():
        for tag in tags:
            ...
            if names is None:
                unknown.append(tag)
                print(f"[fetch] {group}/{tag}: UNKNOWN tag, skipped",
                      flush=True)
                continue
            in_pool = sorted({front.get(n.split(" // ")[0], n)
                              for n in names} & pool)
            tag_names[tag] = in_pool
            hits[tag] = {"scryfall": len(names), "pool": len(in_pool)}
            print(f"[fetch] {group}/{tag}: {len(names)} cards, "
                  f"{len(in_pool)} in pool", flush=True)
    doc = {"pool_version": manifest.get("version"),
           "otag_version": OTAG_VERSION,
           "unknown_tags": unknown, "hits": hits, "tags": tag_names}
    (out_dir / "otags.json").write_text(json.dumps(doc, indent=1) + "\n")
    covered = len({n for ns in tag_names.values() for n in ns})
    print(f"[fetch] {len(tag_names)} tags resolved ({len(unknown)} unknown), "
...
def features(args: argparse.Namespace) -> None:
    ...
    print(f"[ofeat] {len(positions)} positions; tag table covers "
          f"{len(groups_of)} pool cards")
    ...
    for store, wants in sorted(by_store.items()):
        ...
        for g in sorted({g for g, _ in wants}):
            ...
            for g2, t in wants:
                ...
                feats.append(otag_features(traj.decisions[i], traj.header,
                                           seat, groups_of))
    if missed:
        raise SystemExit(f"[ofeat] {len(missed)} positions missed the turn "
                         "join — convention drift, refusing partial dump")
    arr = np.stack(feats)
    np.savez_compressed(
        out_dir / "otag-features.npz",
        keys=np.array(keys), feats=arr,
        feature_names=np.array(FEATURE_NAMES),
        family=np.array([FAMILY_OF[n] for n in FEATURE_NAMES]))
    nz = {n: round(float((arr[:, j] > 0).mean()), 3)
          for j, n in enumerate(FEATURE_NAMES)}
    meta = {"otag_version": OTAG_VERSION, "n_positions": len(keys),
            "nonzero_frac": nz,
            "per_feature_std": {n: round(float(s), 3) for n, s in
                                zip(FEATURE_NAMES, arr.std(0))}}
    (out_dir / "otag-features-meta.json").write_text(
        json.dumps(meta, indent=2) + "\n")
    dead = [n for n, s in zip(FEATURE_NAMES, arr.std(0)) if s == 0]
    print(f"[ofeat] {len(keys)} x {len(FEATURE_NAMES)} in "
