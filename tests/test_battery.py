...
def test_monitor_flags_seq_raw_growth(tmp_path):
    """d6-run14 shape: |seq_raw| grew monotonically across iterations
    (-0.22 -> -0.65 while kl stayed in-band) — the battery flags >3x
    growth so the trend is visible before the guards trip."""
    ...
    run = tmp_path / "run"
    run.mkdir()
    rows = [mrow(k, -0.22 * (1.9**k)) for k in range(4)]  # -0.22 -> -1.5 (~6.9x)
    (run / "monitor.jsonl").write_text("".join(json.dumps(r) + "\n" for r in rows))
    an, _ = battery.monitor_curves(run)
    assert any("seq_raw" in a for a in an), an
    assert not any("kl_mu grew" in a for a in an)
