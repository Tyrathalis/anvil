...
def test_stale_ret_dropped():
    ...
    raw = "".join(json.dumps(r) + "\n" for r in recs).encode()
    _header, decisions, _end, _marks = decode_frame(zstandard.ZstdCompressor().compress(raw))
...
def test_undecodable_frame_quarantined(tmp_path):
    ...
    with pytest.raises(Exception):  # noqa: B017 -- any decode error is a loud failure
