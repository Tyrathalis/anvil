...
def test_batch_chunk_guarantees_four_rounds():
    ...
    generation batch clamps args.chunk so every worker sees >=4 refills (two
    until the fleet bench 09-14: every cell waited ~20 min on its last chunk)."""
    from anvil.training.selfplay import batch_chunk

    assert batch_chunk(240, 16, 30) == 3  # w=16 mirror batch at heur_frac .5
    assert batch_chunk(120, 16, 30) == 1  # w=16 heur seat batch
    assert batch_chunk(240, 8, 30) == 7  # the standard recipe de-tails too
    assert batch_chunk(960, 8, 30) == 30  # big pool: the ceiling holds
    assert batch_chunk(3, 16, 30) == 1  # never zero
