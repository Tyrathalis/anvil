...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            EXPORT.replace("1 Command Tower", "2 Command Tower").replace("96 Forest", "95 Forest"),
            "singleton",
        ),
    ],
)
...
def test_resolve_ladder():
    universe = {
        forge_db.normalize(n): n for n in ["Fire", "Ice", "Command Tower", "Lim-Dul's Vault"]
    }
...
def test_banlist_section_attribution():
    ...
    assert {
        "name": "Edgar Markov",
        "kind": "banned_commander",
        "section": "Banned as Commander only",
    } in cards
...
def test_current_manifest_pin_beats_mtime(tmp_path, monkeypatch):
    ...
    (tmp_path / "pool-aaaa1111.json").write_text(json.dumps({"pool_version": "aaaa1111"}))
    (tmp_path / "pool-bbbb2222.json").write_text(json.dumps({"pool_version": "bbbb2222"}))
...
def test_verify_installed_decks(tmp_path):
    ...
    problems = verify_installed_decks(
        ["dc-1.dck", "dc-2.dck", "dc-3.dck", "dc-4.dck"], decks_out_dir=src, installed_dir=inst
    )
